...
export function extractWorkspace(root: string): Map<string, MpmFileRecord> {
  ...
  const candidates: MpmCandidateFile[] = [];
  for (const path of files) {
    ...
    candidates.push({ path, content, size });
  }

  for (const file of candidates) {
    records.set(file.path, mapFile(root, file.path, file.content, file.size, known));
  }
  return records;
}

/** One ready-to-map file: content already read (by discovery or refresh). */
export interface MpmCandidateFile {
  path: string;
  content: string;
  size: number;
}
...
/**
 * Extract one file against a fixed set of known paths (#617): the unit of
 * incremental refresh. `known` is the set of mapped paths relations may
 * resolve against (the full projection on refresh; the discovered set on
 * full rebuild). Deterministic and content-only: never throws.
 */
