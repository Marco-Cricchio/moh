...
4. **Grill (if needed).** If the request needs fleshing out, load the `grilling` and `domain-modeling` skills (their SKILL.md files are in your skills index) and grill it into shape a round of questions at a time, sharpening domain terms and updating `CONTEXT.md`/ADRs inline as decisions land.
