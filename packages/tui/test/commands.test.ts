...
describe("new base slash commands (/commands /mode /theme /settings /wayfinder)", () => {
  test("BASE_COMMANDS lists the fifteen base commands alphabetically", () => {
    const names = BASE_COMMANDS.map((c) => c.name);
    expect(names).toEqual(["ask-moh", "commands", "compact", "fork", "help", "mode", "model", "mpm", "reload", "rename", "settings", "theme", "thinking", "tree", "wayfinder", "workflow"]);
...
describe("workflow skill aliases", () => {
  test("aliases only exist while workflow is on", () => {
    ...
    expect(activeCommands({ config: DEFAULT_USER_CONFIG }).map((c) => c.name)).toEqual([
      "ask-moh", "commands", "compact", "fork", "help", "mode", "model", "mpm", "reload", "rename", "settings", "theme", "thinking", "tree", "wayfinder", "workflow",
    ]);
    runSlashCommand("/workflow on", ctx);
