...
describe("new base slash commands (/commands /mode /theme /settings /wayfinder)", () => {
  test("BASE_COMMANDS lists the base commands alphabetically", () => {
    ...
    expect(names).toEqual(["ask-moh", "browser", "commands", "compact", "copy", "fork", "help", "jev", "mode", "model", "mpm", "reload", "rename", "routing", "session", "settings", "theme", "thinking", "tree", "wayfinder", "workflow"]);
...
describe("workflow skill aliases", () => {
  test("aliases only exist while workflow is on", () => {
    ...
    expect(activeCommands({ config: DEFAULT_USER_CONFIG }).map((c) => c.name)).toEqual([
      "ask-moh", "browser", "commands", "compact", "copy", "fork", "help", "jev", "mode", "model", "mpm", "reload", "rename", "routing", "session", "settings", "theme", "thinking", "tree", "wayfinder", "workflow",
    ]);
    runSlashCommand("/workflow on", ctx);
