...
8. **Token posture is hybrid (c), amended by #151**: native OAuth tokens for Anthropic and Google (plan limits apply); OpenAI **attempts** to mint its API key via the RFC 8693 exchange, **best-effort** — codex itself tolerates mint failure, and accounts whose id_token lacks organization/workspace claims cannot mint. On success the minted key rides the existing api-key path; on failure the native OAuth tokens are stored (`grant.minted: false`) and streaming goes through the ChatGPT backend (`https://chatgpt.com/backend-api/codex`, Responses API wire, `originator` header like codex) — the fallback and the common case. Refresh keeps attempting the re-mint; its failure is non-fatal for native grants. Anthropic minting is rejected — the minted key bills Console credits, not the Pro/Max plan.
