...
8. **Overlay geometry (superseded by #183)**: modals (Settings, Commands, Permission, Frontier, Onboarding, AskUser) float above the session via OverlayLayer/Dialog. The layer outside the dialog is transparent and must not move the transcript, input, or bottom bar; the dialog surface itself keeps the themed solid background for readability.
