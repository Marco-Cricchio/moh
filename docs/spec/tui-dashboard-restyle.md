...
8. **Overlay geometry (superseded by #183)**: modals (Settings, Commands, Permission, Frontier, Onboarding, AskUser) float centered above the session via OverlayLayer/Dialog. The layer outside the dialog is transparent and must not move the transcript, input, or bottom bar; a terminal alternate buffer provides full-viewport geometry while preserving native scrollback. The dialog surface itself keeps the themed solid background for readability.
