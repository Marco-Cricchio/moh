...
10. **Capabilities.** Reasoning display works for every provider that emits reasoning. Level selection is offered where the active model declares a supported level map (#256 revision) — or a configuration-declared thinking capability: `capabilities.thinking` on an endpoint profile (format + canonical levels, the openai-compat capability source) or a per-model `capabilities.thinkingModels` override on catalog-backed endpoints. Provider-native level names (e.g. `minimal`) normalize into the canonical scale at the catalog projection: `minimal` counts as canonical `low` when the map carries no `low`; when both are present `low` wins and `minimal` is dropped. Declared levels are intersected with what the declared format's wire can express. Capability is declared (catalog or configuration), never inferred from `reasoning` alone; a model with `reasoning: true` and no map and no declaration still offers no selection. An unsupported stored preference is kept intact, resolved to the provider default, and surfaced visibly (status bar / `/thinking`) — never silently remapped or dropped.
